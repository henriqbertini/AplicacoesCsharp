...
<?php 
	...
	else
	{
		        ...
		        $query_select = mysql_query("SELECT *FROM validacao_kit_tela_itens WHERE ID = '$id'");

		        while($config = mysql_fetch_array($query_select)) 
		        {
		        	
		        	$produto = $config['PRODUTO'];

		        	if($config['STATUS'] == 'HABILITADO')
		        		$status = 'DESABILITADO';
		        	else
		        		$status = 'HABILITADO';	
				...
				$sql = "UPDATE validacao_kit_tela_itens SET STATUS ='$status_uppercase' WHERE ID = '$id'";
				$query = mysql_query($sql) or die();
				 ...
				 {
					 echo "<script>alert('Não foi possível realizar a atualização do registro!');</script>";
					 echo "<script> document.location.href='../cadastro_itens_sap.php?ID=$id_produto';</script>"; 
				 }	
